...
for item_name in image_pickle_names:
	...
	# Save the set
	pickle.dump(data_processed, open(os.path.join(set_name, "data", str(running_index).zfill(5) + ".p"), 'wb'), protocol=2)
	pickle.dump(label_processed, open(os.path.join(set_name, "label", str(running_index).zfill(5) + ".p"), 'wb'), protocol=2)
